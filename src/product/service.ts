...
export default class ProductService {
    ...
    static async createOrderFromProduct(id: number, query: Data) {
        await client.query(`INSERT INTO orders (user_id, order_status) VALUES ('${id}', 'FALSE') `)
        const orderId = await client.query(`SELECT order_id FROM orders WHERE (user_id = ${id})`)
        ...
        return await client.query(productOrders)
    }
}
